  ...
  >
    ...
    >
      <img
        ...
        :src="`https://res.cloudinary.com/mikemcbride/image/upload/c_fill,g_faces:center,h_600,q_90,w_600,f_auto/v1/xmas-${year}/collage/${item.name}.webp`"
      />
    </button>
    <div
      v-show="open"
      class="h-screen w-screen fixed inset-0 flex flex-col items-center justify-center"
    >
      <button
        class="h-screen w-screen absolute inset-0 z-0 bg-emerald-900 bg-opacity-90"
        @click="toggle()"
      ></button>
      <div
        class="relative flex gap-6 items-center justify-center flex-col px-4"
      >
        <div class="flex w-full justify-end">
          <button
            class="text-white p-2 opacity-60 hover:opacity-100"
            @click="toggle()"
          >
        ...
        <div class="flex items-center justify-between w-full mx-auto">
          <button
            @click="loadPrevious()"
            class="inline-flex items-center gap-2 text-white rounded text-sm px-3 py-2 opacity-80 hover:opacity-100"
          >
            <ArrowIconLeft class="h-6 w-auto" /> Prev
          </button>
          <button
            @click="loadNext()"
            class="inline-flex items-center gap-2 text-white rounded text-sm px-3 py-2 opacity-80 hover:opacity-100"
          >
            Next <ArrowIconRight class="h-6 w-auto" />
          </button>
        </div>
      </div>
    </div>
